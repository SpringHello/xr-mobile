<template>
  <div id="balancedetail">
    ...
    <Group class="box">
      ...
      <cell title="负载均衡名称" :value="balanceData.name"></cell>
      <cell title="状态"
            :value="balanceData.status == 5 ? '删除中' : balanceData.status == 6 ? '解绑主机中' :balanceData.status ==7? '绑定主机中':'正常'"></cell>
      <cell title="网络类型" :value="balanceData._internal ? '内网负载均衡' : '公网负载均衡'"></cell>
    ...
    <Group>
      ...
      <cell title="内网端口"
            :value="balanceData.privateport"></cell>
      <cell title="外网端口" :value="balanceData.publicport"></cell>
    </Group>
